...
function showAdoptionSuccess(){
  ...
  window.setTimeout(()=>{
    window.location.href = window.location.pathname + window.location.search.replace(/[?&]adopcion=recibida/g,'').replace(/^&/,'?');
...
if(adoptForm){
  adoptForm.addEventListener('submit', async (e)=>{
    ...
    // Basin recibe el formulario directamente. Esto evita depender de FormSubmit,
    // fetch cross-origin o del comportamiento particular del navegador móvil.
    // Conservamos el formulario y todos sus archivos tal como están.
    try{
      adoptForm.action='https://usebasin.com/f/c42627cdb788';
      adoptForm.method='POST';
      adoptForm.enctype='multipart/form-data';
      HTMLFormElement.prototype.submit.call(adoptForm);
    }catch(err){
      console.error('Error enviando adopción:',err);
      if(msg){msg.className='form-msg error';msg.textContent='No se pudo enviar la solicitud. Intentá nuevamente.';}
